...
$(function () {
    ...
    */
    self.Ingredient = Backbone.Model.extend({
        idAttribute: "Id"
    });
    ...
    “url” provides the endpoint for the RESTFul API. 
    This is all that’s needed to retrieve, create, update, and 
    delete ingredients with Backbone’s simple Model API.
    */
    self.IngredientCollection = Backbone.Collection.extend({
        model: self.Ingredient,
    ...
    ingredientList.
    */
    self.IngredientListView = Backbone.View.extend({
        tagName: 'ul',
        ...
        render: function (eventName) {
            _.each(this.model.models, function (ingredient) {
                var ingrListView = new self.IngredientListItemView({ model: ingredient });
    ...
    Part 2.
    */
    self.IngredientListItemView = Backbone.View.extend({
        tagName: "li",
    ...
    */
    ...
    // Router
    /*
    AppRouter: Provides the entry points 
    ...
    will make sure you can deep-link to a specific ingredient.
    */
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.ingredientList = new self.IngredientCollection();
            this.ingredientListView = new self.IngredientListView({ model: this.ingredientList });
            this.ingredientList.fetch();
            $('#div1').html(this.ingredientListView.render().el);
        },

        ingredientDetails: function (id) {
            //this.ingredient = this.ingredientList.first();
            ...
            $('#div2').html(this.ingredientView.render().el);
        }
    });
